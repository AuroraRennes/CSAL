...
static int do_config_cpu_cti_ptm_extin(int nCpu)
{
    ...
    if(cti)
    {
        ...
        if(ret == 0)
            ret = cs_cti_set_trigout_channels(cti, CS_CTI_TRIGOUT_EXTIN0, (chan_bitmask << etm_trace_enable_chan));
        /* set trigger out for EXTIN1 to be associated with TRIGGER_CHAN */
        if(ret == 0)
            ret = cs_cti_set_trigout_channels(cti, CS_CTI_TRIGOUT_EXTIN1, (chan_bitmask << (etm_trig_chan)));
        /* enable the CTI */
        if(ret == 0) {
...
static int cs_etb_flush_and_stop_trig(cs_device_t in_cti)
{
    ...
    if(cti)
    {
        ret = cs_cti_set_trigout_channels(cti, CS_CTI_ETB_TRIGIN_PORT, (0x1U << etb_trig_chan));
        if(ret == 0)
            ret = cs_cti_enable(cti);
    }
    ...
    if(ret == 0)
    {
        ...
        if(devices.etb != 0)
        {
            ...
            ffcr_val |= CS_ETB_FLFMT_CTRL_TrigEvt | CS_ETB_FLFMT_CTRL_StopFl | CS_ETB_FLFMT_CTRL_FOnTrig;
...
int main(int argc, char** argv)
{
  ...
  if(etb_flush_on_trig)
  {
      ...
      if(cs_device_wait(devices.etb, CS_ETB_FLFMT_STATUS, CS_ETB_FLFMT_STATUS_FtStopped,  CS_REG_WAITBITS_ALL_1, 0, &regval) == 0)
      {
          if(verbose)
