...
/*Mercury xu5-2 has a zynq ultrascale+*/
static int do_registration_mercury_xu5_2(struct cs_devices_t * devices)
{
  /*Dual core Cortex-A53 setup, can be extended to quad core and further two Cortex R5 cores.*/
  enum
  {
    A53_0,
    A53_1
  ...
  /*Device adresses from trm Figure 39-8.*/
  cs_device_t tsgen, funnel0, funnel1, funnel2, etf1, etf2, rep, etr, tpiu, cti0, cti1, cti2, ftm, stm;
  int i;

  if (registration_verbose)
    printf("CSDEMO: Regiptrstering Mercury-xu5-2/Zynq Ultrascale+ CoreSight devices...\n");
  cs_exclude_range(0xFEBE0000, 0xFEC00000); /*Exclude Cortex-R5 devices.*/
  cs_exclude_range(0xFED50000, 0xFEFFFFFF); /*Exclude exclude devices for core 2 and 3 (we only have dual core setup).*/
  cs_register_romtable(0xFE800000);

  /* One PTM & one CTI per core. */
  if (registration_verbose)
    printf("CSDEMO: Registering CPU affinities...\n");
  /* ETMs */
  ...
  /*Both ETMs of the Cortex-A53 feed directly into funnel1, funnel0 is unused and designed for Cortex-R5.*/
  /*We make the assumption that ETM{0|1} feed to corresponding port {0|1}, although not explicitly stated in the trm.*/
  ...
  /* Funnel0 feeds directly into funnel2 on port 0, ETF1 feeds into funnel2 on port 2, and STM feeds into funnel2 on port2. */
  /*Assumption: port numbers, not explcitly stated in trm.*/
  funnel2  = cs_device_get(0xFE930000); 
  cs_atb_register(etf1, 0, funnel2, 2);


  /* STM feeds into funnel 2.*/
  cs_atb_register(stm, 0, funnel2, 0);

  /*Funnel2 feeds into ETF2*/
  etf2 = cs_device_get(0xFE950000);
  cs_atb_register(funnel2, 0, etf2, 0);

  /*ETF2 feeds into the replicator, outputting either to TPIU or ETR*/
  rep = cs_atb_add_replicator(2);
  cs_atb_register(etf2, 0, rep, 0);
    
  etr = cs_device_get(0xFE970000); /*In our case we have an ETR.*/
  devices -> tpiu = cs_device_get(0xFE980000);
  
  /*Assumption: Port numbers.*/
  cs_atb_register(rep,0,etr,0);
  cs_atb_register(rep,2,devices -> tpiu,0);

  /*
    We use ETR as our main buffer to read trace from. 
  */
  devices -> etb = etr;
  ...
  devices -> tsgen = tsgen;
  /*
    Hardcode CPU-IDs as Cortex-A53.
  */
...

{
  ...
  if (registration_verbose)
    printf("CSDEMO: Registering Zynq7000 CoreSight devices...\n");
  ...
  /* PTM{0,1} are on port 0&1, the FTM (not initialised here) is on port 2,
   * and the ITM is on port 3. */
  if (registration_verbose)
    printf("CSDEMO: Registering trace-bus connections...\n");
  ...
  /* The replicator splits the funnel output between ETB (port 0) and TPIU
   * (port 1). */
  rep = cs_atb_add_replicator(2);
  cs_atb_register(funnel, 0, rep, 0);
  devices->etb = cs_device_get(0xF8801000);
  devices->tpiu = cs_device_get(0xF8803000);
  cs_atb_register(rep, 0, devices->etb, 0);
  cs_atb_register(rep, 1, devices->tpiu, 0);

  /* As for the Snowball, the Zynq has a single CPU Part entry to cover both
   * cores so cannot use the probe board results. */
  for (i = 0; i < 2; ++i)
    devices->cpu_id[i] = 0xC09;
...
static int do_registration_arndale(struct cs_devices_t *devices)
{
  ...
  cs_device_t rep_main, rep_itm, funnel;
  int i;
  ...
  cs_atb_register(cs_cpu_get_device(A15_0, CS_DEVCLASS_SOURCE), 0, funnel, 0);
  cs_atb_register(cs_cpu_get_device(A15_1, CS_DEVCLASS_SOURCE), 0, funnel, 1);

  rep_main = cs_atb_add_replicator(2);
  cs_atb_register(funnel, 0, rep_main, 0);
  devices->etb = cs_device_get(0x10881000);
  devices->tpiu = cs_device_get(0x10883000);
  cs_atb_register(rep_main, 0, devices->etb, 0);
  cs_atb_register(rep_main, 1, devices->tpiu, 0);
...
static int do_registration_tc2(struct cs_devices_t *devices)
{
  ...
  cs_device_t rep_main, rep_itm, cscti, funnel;
  int i;
  ...
  cs_atb_register(cs_cpu_get_device(A15_0, CS_DEVCLASS_SOURCE), 0, funnel, 0);
  cs_atb_register(cs_cpu_get_device(A15_1, CS_DEVCLASS_SOURCE), 0, funnel, 1);
  cs_atb_register(cs_cpu_get_device(A7_0, CS_DEVCLASS_SOURCE), 0, funnel, 2);
  /* 3 is the ITM (or one of its replicator outputs) */
  cs_atb_register(cs_cpu_get_device(A7_1, CS_DEVCLASS_SOURCE), 0, funnel, 4);
  cs_atb_register(cs_cpu_get_device(A7_2, CS_DEVCLASS_SOURCE), 0, funnel, 5);

  rep_main = cs_atb_add_replicator(2);
  cs_atb_register(funnel, 0, rep_main, 0);
  devices->etb = cs_device_get(0x20010000);
  devices->tpiu = cs_device_get(0x20030000);
  cs_atb_register(rep_main, 0, devices->etb, 0);
  cs_atb_register(rep_main, 1, devices->tpiu, 0);
...
static int do_registration_juno(struct cs_devices_t *devices)
{
  ...
  cs_device_t rep, etr, etf, fun_main, fun_a53, fun_a57, stm, sys_cti;
  cs_device_t r1_fun_scp, r1_fun_common, r1_etf_scp, r1_cti_2, ela_a53, ela_a57;
  int i;
  ...
  cs_atb_register(rep, 0, devices->tpiu, 0);
  ...
  */
  ...
  cs_cti_connect_trigdst(cs_cti_trigdst(sys_cti, 7), etf, CS_TRIGIN_ETB_FLUSHIN);
  cs_cti_connect_trigdst(cs_cti_trigdst(sys_cti, 6), etf, CS_TRIGIN_ETB_TRIGIN);
  ...
  cs_cti_connect_trigdst(cs_cti_trigdst(sys_cti, 0), etr, CS_TRIGIN_ETB_FLUSHIN);
  cs_cti_connect_trigdst(cs_cti_trigdst(sys_cti, 1), etr, CS_TRIGIN_ETB_TRIGIN);
...
static int do_registration_altera(struct cs_devices_t *devices)
{
  ...
  devices->tpiu = cs_device_get(0xff003000);
  cs_atb_register(rep, 1, devices->tpiu, 0);
...
#ifdef CS_PA64BIT
// this board needs 64 bit physical address
...
const struct board known_boards[] = {
    ...
    {
        .do_registration = do_registration_mercury_xu5_2,
        .n_cpu = 1,
        .hardware = "MERCURY-ZYNQ-ULTRASCALE+",
    },
    {
