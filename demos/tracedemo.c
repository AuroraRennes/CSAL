...
static bool trace_cycle_accurate_ccit;
static unsigned ccit; /*cycle-count interval*/
static bool test_tpiu;
static bool tpiu_con;
static bool branch_broadcasting = false;
#define BOARD_NAME_LEN 256
static char board_name[BOARD_NAME_LEN];
static unsigned n_pmu_events = 0;
static unsigned int pmu_events[MAX_PMU_EVENTS];
...
// static unsigned long o_trace_start_address = 0x0;
// static unsigned long o_trace_end_address = 0xFFFFFFFFFFFFFFFF; //Maxint
...
static int do_init_etm(cs_device_t dev) {
  ...
  if (!CS_ETMVERSION_IS_ETMV4(etm_version)) {
    ...
    v4config.flags |= CS_ETMC_TRACE_ENABLE;
    /* trace enable */
    if(itm_only) {
    ...
    if(branch_broadcasting){
      v4config.configr.bits.bb = 1;
    }
...
static void show_etm_config(unsigned int n)
{
  ...
  if(CS_ETMVERSION_MAJOR(cs_etm_get_version(devices.ptm[n])) >= CS_ETMVERSION_ETMv4)
    p_config = &t4config;
  else
    p_config = &tconfig;
...
static int do_config_pmu(int n_core){
  int rc = -1;
  cs_device_t  pmu = devices.pmu[n_core];
  /*TODO: complete this to setup PMU, we did manually for now.*/
  
  // rc = cs_pmu_write_status(pmu, CS_PMU_DIV64|CS_PMU_EVENTTYPES, &);
  // if (rc){
  //   fprintf(stderr, "Failed to write status trace for CPU #%u\n", n_core);
  // }
  rc =  cs_pmu_reset(pmu, CS_PMU_CYCLES|CS_PMU_OVERFLOW|CS_PMU_ENABLE);
  if (rc){
      fprintf(stderr, "Failed to configure PMU trace for CPU #%u\n", n_core);
  
  }

  rc =  cs_pmu_bus_export(pmu, 0x1);
  if (rc){
      fprintf(stderr, "Failed to configure PMU trace for CPU #%u\n to export events to bus", n_core);
  }

  // if(!cs_pmu_is_enabled(pmu)){
...
//      tconfig.addr_comp[1].access_type = CS_ETMACT_EX|CS_ETMACT_ARMTHUMB|CS_ETMACT_USER;
    ...
    tconfig.sequencer.transition_event[CS_ETMSQOFF(1,2)] = CS_ETMER_SAC(0);
    tconfig.sequencer.transition_event[CS_ETMSQOFF(2,3)] = CS_ETMER_SAC(1);
...
static int do_config_etmv4(int n_core)
{
  ...
  tconfig.flags = CS_ETMC_TRACE_ENABLE | CS_ETMC_CONFIG | CS_ETMC_EVENTSELECT | CS_ETMC_RES_SEL ;
  ...
  {
    unsigned int i;
    tconfig.eventctlr0r = 0;
    tconfig.eventctlr1r = 0;
    tconfig.extinselr = 0;
    for (i = 0; i < n_pmu_events; ++i) {
      printf("pmu events[i] = 0x%x\n", pmu_events[i]);
      tconfig.extinselr |= (pmu_events[i] << (8*i));   /* ext[i] := selected PMU event */
      tconfig.rsctlr[i+2] = (1U << (i)); /* resource[i+2] = ext[i] */ /* HERE THERES WAS A BUG IN THE CODE FROM CSAL (atleast for Cortex-A53) -- Matthew Weingarten*/
      tconfig.rsctlr_acc_mask |= (1U << (i+2));
      tconfig.eventctlr0r |= ((i+2) << (8*i));  /* event[i] := resource[i+2] */
      tconfig.eventctlr1r |= (1U << i);     /* generate event in inst stream */
    }
  }

  cs_etm_config_print_ex(etm, &tconfig);
  
  cs_etm_enable_programming(etm);
...
printf("CSDEMO: Configuring trace...\n");
  ...
  for (i = 0; i < board->n_cpu; ++i) {
    ...
    printf("CSDEMO: Configuring PMU id for CPU #%d PMU...\n", i);
    devices.pmu[i] = cs_cpu_get_device(i, CS_DEVCLASS_PMU);
    
    if (devices.ptm[i] == CS_ERRDESC) {
      fprintf(stderr, "** Failed to get PMU for CPU #%d\n", i);
      return -1;
    }
  ...
  for (i = 0; i < board->n_cpu; ++i) {
      ...
      /* Config PMU if we want to have events in the trace stream --- Matthew Weingarten*/
      if(n_pmu_events > 0){
        // r = do_config_pmu(i); /*TODO*/
      }
  ...
  if(devices.tpiu != NULL){
    if(cs_sink_enable(devices.tpiu)){
      printf("CSDEMO: Could not enable TPIU - not running demo\n");
      return -1;
    }
  }

  /*
    If we use the devices.trace_sinks as hardware fifo that are on the path
    between sources and sink, it is important to make sure they are in order,
    this way no trace data will get lost in iterative flushing of buffers. 
  */
  for(i = 0; i < devices.n_trace_sinks;++i){
    cs_device_t * tmc_hw_fifo = devices.trace_sinks[i];
    if(cs_tmc_hw_fifo_enable(tmc_hw_fifo, /*bufwm=*/0x0) != 0){
      printf("CSDEMO: Could not enable sinks as hw fifo - not running demo\n");
      return -1;
    }
    // dump_tmc_config(tmc_hw_fifo);
  }
  ...
  if(trace_timestamps){
    cs_trace_enable_timestamps(devices.tsgen, 1);
  }
  ...
  for (i = 0; i < board->n_cpu; ++i) {
    ...
    if(trace_cycle_accurate_ccit){

      if(cs_trace_enable_cycle_accurate_ccit(devices.ptm[i], 1, ccit)){
        return -1;
      } 
    }
...
static int help(void)
{
  ...
  printf("-cycle-accurate\tEnable Cycle Accurate tracing with minimal interval\n");
  printf("-ccit-cycle-accurate <ccit>\tEnable Cycle Accurate tracing and set ccit \n");
  ...
  printf("-branch-broadcasting\t Enable branch broadcasting\n");
  printf("-<event>\t To enable tracing <event> PMU events  in the trace stream. Can add up to  %u event options. Example is -L1I_CACHE_REFILL -L1I_CACHE to trace both instruction cache misses and instructions cache accesses and embedd them int the trace stream - Possible events : [L1D_CACHE_ALLOCATE, CHAIN, BUS_CYCLES, TTBR_WRITE_RETIRED, INST_SPEC, MEMORY_ERROR, BUS_ACCESS, L2D_CACHE_WB, L2D_CACHE_REFILL, L2D_CACHE, L1D_CACHE_WB, L1I_CACHE, MEM_ACCESS, BR_PRED, CPU_CYCLES, BR_MIS_PRED, UNALIGNED_LDST_RETIRED, BR_RETURN_RETIRED, BR_IMMED_RETIRED, PC_WRITE_RETIRED, CID_WRITE_RETIRED, EXC_RETURN, EXC_TAKEN, INST_RETIRED, ST_RETIRED, LD_RETIRED, L1D_TLB_REFILL, L1D_CACHE, L1D_CACHE_REFILL, L1I_TLB_REFILL, L1I_CACHE_REFILL, SW_INCR]\n", MAX_PMU_EVENTS);
  printf("-fon-stop\tEnable ETB stop on flush. Uses manual flush to halt ETB trace collection\n");
  printf("-pause\tRun the demo with a pause after each step.\n");
  printf("-test-tpiu\tRun the demo and setup TPIU in continuous testing mode.\n");
  printf("-tpiu-con\tRun the demo and setup TPIU in continuous mode. (Otherwise will run normal mode)\n");
  return EXIT_FAILURE;
}


static int resolve_event(const char* event){
   if (!strcasecmp("L1D_CACHE_ALLOCATE", event)) {return L1D_CACHE_ALLOCATE;}  
   else if (!strcasecmp("CHAIN", event)) {return CHAIN;}  
   else if (!strcasecmp("BUS_CYCLES", event)) {return BUS_CYCLES;}  
   else if (!strcasecmp("TTBR_WRITE_RETIRED", event)) {return TTBR_WRITE_RETIRED;}  
   else if (!strcasecmp("INST_SPEC", event)) {return INST_SPEC;}  
   else if (!strcasecmp("MEMORY_ERROR", event)) {return MEMORY_ERROR;}  
   else if (!strcasecmp("BUS_ACCESS", event)) {return BUS_ACCESS;}  
   else if (!strcasecmp("L2D_CACHE_WB", event)) {return L2D_CACHE_WB;}  
   else if (!strcasecmp("L2D_CACHE_REFILL", event)) {return L2D_CACHE_REFILL;}  
   else if (!strcasecmp("L2D_CACHE", event)) {return L2D_CACHE;}  
   else if (!strcasecmp("L1D_CACHE_WB", event)) {return L1D_CACHE_WB;}  
   else if (!strcasecmp("L1I_CACHE", event)) {return L1I_CACHE;}  
   else if (!strcasecmp("MEM_ACCESS", event)) {return MEM_ACCESS;}  
   else if (!strcasecmp("BR_PRED", event)) {return BR_PRED;}  
   else if (!strcasecmp("CPU_CYCLES", event)) {return CPU_CYCLES;}  
   else if (!strcasecmp("BR_MIS_PRED", event)) {return BR_MIS_PRED;}  
   else if (!strcasecmp("UNALIGNED_LDST_RETIRED", event)) {return UNALIGNED_LDST_RETIRED;}  
   else if (!strcasecmp("BR_RETURN_RETIRED", event)) {return BR_RETURN_RETIRED;}  
   else if (!strcasecmp("BR_IMMED_RETIRED", event)) {return BR_IMMED_RETIRED;}  
   else if (!strcasecmp("PC_WRITE_RETIRED", event)) {return PC_WRITE_RETIRED;}  
   else if (!strcasecmp("CID_WRITE_RETIRED", event)) {return CID_WRITE_RETIRED;}  
   else if (!strcasecmp("EXC_RETURN", event)) {return EXC_RETURN;}  
   else if (!strcasecmp("EXC_TAKEN", event)) {return EXC_TAKEN;}  
   else if (!strcasecmp("INST_RETIRED", event)) {return INST_RETIRED;}  
   else if (!strcasecmp("ST_RETIRED", event)) {return ST_RETIRED;}  
   else if (!strcasecmp("LD_RETIRED", event)) {return LD_RETIRED;}  
   else if (!strcasecmp("L1D_TLB_REFILL", event)) {return L1D_TLB_REFILL;}  
   else if (!strcasecmp("L1D_CACHE", event)) {return L1D_CACHE;}  
   else if (!strcasecmp("L1D_CACHE_REFILL", event)) {return L1D_CACHE_REFILL;}  
   else if (!strcasecmp("L1I_TLB_REFILL", event)) {return L1I_TLB_REFILL;}  
   else if (!strcasecmp("L1I_CACHE_REFILL", event)) {return L1I_CACHE_REFILL;}  
   else if (!strcasecmp("SW_INCR", event)) {return L2D_CACHE;}  
   else {return -1;}
}
...
void kill_handler(int sig){
    ...
    for(i = 0; i < devices.n_trace_sinks;++i){
      cs_device_t * tmc_hw_fifo = devices.trace_sinks[i];
      cs_tmc_hw_fifo_disable(tmc_hw_fifo);
    }
...
int main(int argc, char** argv)
{
  ...
  if (argc >= 2) {
    ...
    for (; i < argc; ++i) {
      ...
      if (opt[0] != '-') {
        fprintf(stderr, "Unknown argument \"%s\", use -h for help:\n", opt);
        return help();
      } else {
        ++opt;
        if (opt[0] == '-') {
        ...
        unsigned int pmu_event = resolve_event(opt);
        if (strcmp(opt, "c") == 0) {
          if (i + 1 < argc) {
        ...
        } else if (strcmp(opt, "test-tpiu") == 0) {
          test_tpiu = true;
        }
        else if (strcmp(opt, "tpiu-con") == 0) {
          tpiu_con = true;
        }
        else if (strcmp(opt, "branch-broadcasting") == 0) {
          branch_broadcasting = true;
        }
        else if(pmu_event != -1 ){ /*This is an event option being turned on*/
          if (n_pmu_events >= 4){
            fprintf(stderr, "Cannot trace more than \"%d\" events. \n", MAX_PMU_EVENTS);
            return help();
          }
          printf("Tracing HW event: 0x%x\n",pmu_event);
          pmu_events[n_pmu_events] = pmu_event;
          printf("Tracing HW event: 0x%x\n",pmu_events[n_pmu_events]);
          n_pmu_events += 1;
        }
        else {
  ...
  if (!(o_trace_start_address < o_trace_end_address)) {
    fprintf(stderr, "** trace end address 0x%lx must be greater than trace start address 0x%lx\n",
  ...
  /*This is not so nice, but we are making sure that if we send a kill to the demo it properly shuts itself down*/
  ...
  if(test_tpiu){
    if(cs_tpiu_test_mode_enable(devices.tpiu)){
      printf("CSDEMO: Failed to enable TPIU test mode - not running demo\n");
    }
    if(cs_sink_enable(devices.tpiu)){
      printf("CSDEMO: Failed to enable TPIU - not running demo\n");
    }
    printf("Running TPIU in test mode...\n");
    printf("Current test mode = 0x%lx \n", cs_device_read(devices.tpiu, CS_TPIU_CTEST));
    printf("Test pattern repeat counter = 0x%lx \n", cs_device_read(devices.tpiu, CS_TPIU_TESTCOUNT));
    await_user_stop();
    if(cs_tpiu_test_mode_disable(devices.tpiu)){
      printf("CSDEMO: Failed to disable TPIU test mode - not running demo\n");
    }
    if(cs_sink_disable(devices.tpiu)){
      printf("CSDEMO: Failed to disable TPIU - not running demo\n");
    }

    printf("Done running TPIU in test mode\n");
    return EXIT_SUCCESS;
  }
