  ...
  if (d->type == DEV_ETB || d->type == DEV_ETF) {
    ...
    */
    ...
    if (d->v.etb.is_tmc_device) {
      ...
      /* Set mode to Circular buffer if tmc. */
       _cs_write(d, CS_TMC_MODE, CS_TMC_MODE_CIRCULAR);
    }
...
int cs_tpiu_test_mode_enable(cs_device_t dev){
  struct cs_device *d = DEV(dev);
  assert(cs_device_has_class(dev, CS_DEVCLASS_SINK));
  assert (d -> type ==  DEV_TPIU);
  _cs_unlock(d);
  _cs_write(d, CS_TPIU_TESTCOUNT,  0xFF);
  return _cs_write(d, CS_TPIU_CTEST, CS_TPIU_TEST_PContEn |
                                CS_TPIU_TEST_PatF0 |
                                CS_TPIU_TEST_PatA5 |
                                CS_TPIU_TEST_PatW0 |
                                CS_TPIU_TEST_PatW1 );
...
int cs_tpiu_test_mode_disable(cs_device_t dev){
    struct cs_device *d = DEV(dev);
    assert(cs_device_has_class(dev, CS_DEVCLASS_SINK));
    assert (d -> type ==  DEV_TPIU);
    _cs_unlock(d);
    _cs_write(d, CS_TPIU_TESTCOUNT, 0x0);
    return _cs_write(d, CS_TPIU_CTEST, 0x0);
}
...
void dump_tmc_config(cs_device_t * dev){
    ...
    printf("CS_ETB_PSCR:0x%x\n",_cs_read(d, CS_ETB_PSCR));
    printf("----------------------------------\n");
}
