...
int _cs_etm_v4_config_get(struct cs_device *d, cs_etmv4_config_t *c)
{
        ...
        if(c->scv4->idr4.bits.numsscc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->ss_comps_acc_mask & masksel)
                {
                    ...
                    c->ss_comps[i].sspcicr = _cs_read(d, CS_ETMV4_SSPCICR(i));
                }
                masksel <<= 1;
            }
        }
    ...
    if(c->flags & CS_ETMC_ADDR_COMP)
    {
        if(c->scv4->idr4.bits.numacpairs > 0)
        {
            ...
            a_size = (c->scv4->idr2.bits.dasize == 0x8) ? 64 : ((c->scv4->idr2.bits.iasize == 0x8) ? 64 : 32);
            masksel = 0x1;
            for(i = 0; i < numregs; i++)
            {
                if(c->addr_comps_acc_mask & masksel)
                {
                    c->addr_comps[i].acvr_l = _cs_read(d,CS_ETMV4_ACVR(i));
                    if(a_size == 64)
                        c->addr_comps[i].acvr_h = _cs_read(d,CS_ETMV4_ACVR(i)+4);
                    else
                        c->addr_comps[i].acvr_h = 0;
                    c->addr_comps[i].acatr_l = _cs_read(d, CS_ETMV4_ACATR(i));                
                }
                masksel <<= 1;
            }
    ...
    if(c->flags & CS_ETMC_DATA_COMP)
    {
        if(c->scv4->idr4.bits.numdvc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                ...
                {
                    c->data_comps[i].dvcvr_l = _cs_read(d,CS_ETMV4_DVCVR(i));
                    c->data_comps[i].dvcmr_l = _cs_read(d, CS_ETMV4_DVCMR(i));                
                    if(a_size == 64)
                    {
                        c->data_comps[i].dvcvr_h = _cs_read(d,CS_ETMV4_DVCVR(i)+4);
                        c->data_comps[i].dvcmr_h = _cs_read(d, CS_ETMV4_DVCMR(i)+4);                
                    }
                    else
                    {
    ...
    if(c->flags & CS_ETMC_CXID_COMP)
    {
        if(c->scv4->idr4.bits.numcidc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                ...
                {
                    c->cxid_comps[i].cidcvr_l = _cs_read(d,CS_ETMV4_CIDCVR(i));
                    if(a_size == 64)
                    {
                        c->cxid_comps[i].cidcvr_h = _cs_read(d,CS_ETMV4_CIDCVR(i)+4);
                    }
                    else
                    {
    ...
    if(c->flags & CS_ETMC_VMID_COMP)
    {
        if(c->scv4->idr4.bits.numvmidc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                ...
                {
                    c->vmid_comps[i].vmidcvr_l = _cs_read(d,CS_ETMV4_VMIDCVR(i));
                    if(a_size == 64)
                    {
                        c->vmid_comps[i].vmidcvr_h = _cs_read(d,CS_ETMV4_VMIDCVR(i)+4);
                    }
                    else
                    {
...
int _cs_etm_v4_config_put(struct cs_device *d, cs_etmv4_config_t *c)
{
   ...
   /* general configuration */
    if(c->flags & CS_ETMC_CONFIG)
    {
        ...
        _cs_write(d,CS_ETMV4_TRACEIDR, c->traceidr & BITMASK(c->scv4->idr5.bits.traceidsize));
    }
        ...
        if(c->scv4->idr4.bits.numsscc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->ss_comps_acc_mask & masksel)
                {
                    ...
                    _cs_write(d, CS_ETMV4_SSPCICR(i), c->ss_comps[i].sspcicr);
                }
                masksel <<= 1;
            }
        }
    ...
    if(c->flags & CS_ETMC_ADDR_COMP)
    {
        if(c->scv4->idr4.bits.numacpairs > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->addr_comps_acc_mask & masksel)
                {
                    _cs_write(d,CS_ETMV4_ACVR(i), c->addr_comps[i].acvr_l);
                    _cs_write(d,CS_ETMV4_ACVR(i)+4,  c->addr_comps[i].acvr_h /*& a_mask*/);
                    _cs_write(d, CS_ETMV4_ACATR(i), c->addr_comps[i].acatr_l );                    
                }
                masksel <<= 1;
            }
        }
        ...
        if(c->scv4->idr4.bits.numdvc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->data_comps_acc_mask & masksel)
                {
                    ...
                    _cs_write(d, CS_ETMV4_DVCVR(i), c->data_comps[i].dvcvr_l & ~c->data_comps[i].dvcmr_l );
                    _cs_write(d, CS_ETMV4_DVCMR(i), c->data_comps[i].dvcmr_l );                
                    if(a_size == 64)
                    {
                        _cs_write(d,CS_ETMV4_DVCVR(i)+4, c->data_comps[i].dvcvr_h & ~c->data_comps[i].dvcmr_h );
                        _cs_write(d, CS_ETMV4_DVCMR(i)+4, c->data_comps[i].dvcmr_h );                
                    }
                }
                masksel <<= 1;
            }
    ...
    if(c->flags & CS_ETMC_CXID_COMP)
    {
        if(c->scv4->idr4.bits.numcidc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->cxid_comps_acc_mask & masksel)
                {
                    _cs_write(d,CS_ETMV4_CIDCVR(i), c->cxid_comps[i].cidcvr_l);
                    if(a_size == 64)
                    {
                        _cs_write(d,CS_ETMV4_CIDCVR(i)+4, c->cxid_comps[i].cidcvr_h);
                    }
                }
                masksel <<= 1;
            }
...
int _etmv4_edesc(unsigned int eventval, char *pstr)
{
    ...
    if(eventval & 0x80)
    {
        ...
        if(regidx != 0)
        {
            chprinted = sprintf(pstr, "[Pair]: RSCTLR%i RSCTLR%i",regidx*2, regidx*2+1);
        }
        else
    ...
    else
    {
        ...
        if(regidx > 1)
            ...
            chprinted = sprintf(pstr, "[Single]: %s",regidx==1 ? "ALWAYS" : "NEVER");
    }
...
int _sel_desc(int regnum, unsigned int selector, char *pstr)
{
    ...
    else
    {
        ...
        case 1:
            if(select <= 0xFF) /* 8 valid bits */
            {
                ...
                ch_printed += sprintf(pstr+ch_printed, "PE Comp input %s",buf1);
                valid = 1;
            }
            break;
        ...
        case 2:
            if(select <= 0xFF) /* 8 valid bits */
            {
                valid = 1;
                _bitnums_as_str(4,select, buf1);
                ch_printed += sprintf(pstr+ch_printed, "CountZ { %s };", buf1);
                _bitnums_as_str(4,select>>4,buf1);
                ch_printed += sprintf(pstr+ch_printed, " Seq States { %s }", buf1);
        ...
        case 3:
            if(select <= 0xFF) /* 8 valid bits */
            {
                valid = 1;
                _bitnums_as_str(8,select, buf1);
                ch_printed += sprintf(pstr+ch_printed, "Single Shot Comp: %s ",buf1);
            }
            break;
        ...
        case 4: /* 16 valid bits */
            ...
            ch_printed += sprintf(pstr+ch_printed,"Single Addr Comp: %s",buf1);
            break;
        ...
        case 5: 
            if(select <= 0xFF) /* 8 valid bits */
            {
                valid = 1;
                _bitnums_as_str(8,select,buf1);
                ch_printed += sprintf(pstr+ch_printed,"Addr Range Comp: %s",buf1);
            }
            break;
        ...
        case 6:
            if(select <= 0xFF) /* 8 valid bits */
            {
                valid = 1;
                _bitnums_as_str(8,select,buf1);
                ch_printed += sprintf(pstr+ch_printed,"Cxt ID Comp: %s",buf1);
            }
            break;
        ...
        case 7:
            if(select <= 0xFF) /* 8 valid bits */
            {
                valid = 1;
                _bitnums_as_str(8,select,buf1);
                ch_printed += sprintf(pstr+ch_printed,"VMID Comp: %s",buf1);
            }
            break;
    ...
    if(valid)
    {
        ch_printed += sprintf(pstr+ch_printed," %s%s", inv ? "INV " : "", pairinv ? " PAIRINV" : "");
    }
    return ch_printed;
}
...
int _cs_etm_v4_config_print(struct cs_device *d, cs_etmv4_config_t *c)
{
    ...
    if(c->flags & CS_ETMC_CONFIG)
    {
        ...
        printf("\tTRACEIDR = %08X\n", c->traceidr);
        printf("\tCONFIGR = %08X\n", c->configr.reg);
        printf("\t\t.instp0 = %X (%s)\n",c->configr.bits.instp0,inst_p0_sz[c->configr.bits.instp0]);
        printf("\t\t.bb     = %X (Branch Broadcast %s)\n", c->configr.bits.bb, c->configr.bits.bb==1 ? "enabled" : "disabled");
        printf("\t\t.cci    = %X (Cycle Count instruction trace %s)\n",c->configr.bits.cci, c->configr.bits.cci==1 ? "enabled" : "disabled");
        printf("\t\t.cid    = %X (Context ID trace %s)\n",c->configr.bits.cid, c->configr.bits.cid==1 ? "enabled" : "disabled");
        printf("\t\t.vmid   = %X (VMID trace %s)\n",c->configr.bits.vmid, c->configr.bits.vmid==1 ? "enabled" : "disabled");
        printf("\t\t.cond   = %X (Conditional instuction trace: %s)\n",c->configr.bits.cond,cond_sz[c->configr.bits.cond]);
        printf("\t\t.ts     = %X (Global timestamp trace %s)\n",c->configr.bits.ts, c->configr.bits.ts==1 ? "enabled" : "disabled");
        printf("\t\t.rs     = %X (Return Stack %s)\n",c->configr.bits.rs, c->configr.bits.rs==1 ? "enabled" : "disabled");
        printf("\t\t.qe     = %X (Q element trace: %s\n",c->configr.bits.qe,qe_sz[c->configr.bits.qe]);
        printf("\t\t.da     = %X (Data address trace %s)\n",c->configr.bits.da, c->configr.bits.da==1 ? "enabled" : "disabled");
        printf("\t\t.dv     = %X (Data value trace %s)\n",c->configr.bits.dv, c->configr.bits.dv==1 ? "enabled" : "disabled");
    ...
    if(c->flags & CS_ETMC_EVENTSELECT)
    {
        ...
        printf("\t\t.insten = %01X: Instr Trace event element on events - ",c->eventctlr1r & 0xF);
        ...
        printf("\t\t.dataen = %d: Data trace element %s\n",(c->eventctlr1r >> 4) & 0x1, c->eventctlr1r & 0x10 ? "on event 0" : "disabled");
        printf("\t\t.atb = %d: ATB trace trigger %s\n", (c->eventctlr1r >> 11) & 0x1, (c->eventctlr1r >> 11) & 0x1 ? "enabled" : "disabled");
    ...
    if(c->flags & CS_ETMC_SSHOT_CTRL)
    {
        if(c->scv4->idr4.bits.numsscc > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->ss_comps_acc_mask & masksel)
                {
                    _bitnums_as_str(8,(c->ss_comps[i].ssccr >> 16) & 0xFF,sz_buf2);  
    ...
    if(c->flags & CS_ETMC_ADDR_COMP)
    {
        static char const *const tname[4] = { "I Addr", "D LD Addr", "D ST Addr", "D LD or ST Addr" };
        static char const *const ctxttname[4] = { "None", "CXTID", "VMID", "CXTID & VMID" };
        static char const *const dm_name[4] = { "None", "Identical", "Reserved", "Different" };
        static char const *const ds_name[4] = { "Byte", "Halfword", "Word", "DoubleWord" };
        ...
        if(c->scv4->idr4.bits.numacpairs > 0)
        {
            ...
            for(i = 0; i < numregs; i++)
            {
                if(c->addr_comps_acc_mask & masksel)
                {
                    ...
                    {
                        if(i%2 == 0)
                            printf("Data: Match-%s; Size-%s; %s )\n",dm_name[(c->addr_comps[i].acatr_l >> 16 )&0x3],ds_name[(c->addr_comps[i].acatr_l >> 18 )&0x3], (c->addr_comps[i].acatr_l & 0x100000) ? "Range AC" : "Single AC"); 
                        else
