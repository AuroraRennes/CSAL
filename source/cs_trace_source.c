...
int cs_set_trace_source_id(cs_device_t dev, cs_atid_t id)
{
  ...
  } else if (d->type == DEV_FTM) {
      /* note: bits 6:4 are used from this register
               bits 3:0 are ORed with FTMD_TRACEIN_ATID pins from FPGA input */
      _cs_set_mask(d, CS_FTM_ATID, CS_FTM_ATID_BITMASK, (unsigned int)id);
  } else {
    return -1;
  }
...
cs_atid_t cs_get_trace_source_id(cs_device_t dev)
{
  ...
  if (cs_device_has_class(dev, CS_DEVCLASS_CPU)) {
    ...
    id = _cs_read(d, CS_FTM_ATID) & CS_FTM_ATID_BITMASK;
  } else {
...
int cs_trace_enable(cs_device_t dev)
{
  ...
  if (cs_device_has_class(dev, CS_DEVCLASS_CPU)) {
    ...
    _cs_set(d, CS_FTM_GLBCTRL, CS_FTM_GLBCTRL_FTMENABLE_BITMASK);
    _cs_set(d, CS_FTM_CONTROL, CS_FTM_CONTROL_TRACEN_BITMASK);
  } else {
    /* TBD */
  }
  return 0;
...
int cs_trace_disable(cs_device_t dev)
{
  ...
  if (cs_device_has_class(dev, CS_DEVCLASS_CPU)) {
    if(CS_ETMVERSION_MAJOR(_cs_etm_version(d)) >= CS_ETMVERSION_ETMv4) {
      ...
      _cs_clear(d, CS_FTM_CONTROL, CS_FTM_CONTROL_TRACEN_BITMASK);
      _cs_clear(d, CS_FTM_GLBCTRL, CS_FTM_GLBCTRL_FTMENABLE_BITMASK);
  } else {
    /* TBD: */
  ...
  if (d->type == DEV_ETM) {
    ...
    if (CS_ETMVERSION_MAJOR(_cs_etm_version(d)) >= CS_ETMVERSION_ETMv4) {
      ...
      if (enable) {
        unsigned int const CCITMIN = _cs_read(d, CS_ETMv4_IDR3) & 0xfff;
        if(ccit < CCITMIN){
          fprintf(stderr, "Cycle-count interval ccit %u was lower than the CCITMIN %u\n", ccit,CCITMIN);
          return -1;
        }else{
          _cs_write(d, CS_ETMV4_CCCTLR, ccit);
        }
      }
      /* enable cycle count on instruction trace */
      return _cs_set_bit(d, CS_ETMV4_CONFIGR, CS_ETMV4_CONFIGR_CCI, enable);
