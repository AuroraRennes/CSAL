  ...
  if (cs_device_has_class(dev, CS_DEVCLASS_SOURCE)) {
    ...
    if (cs_device_has_class(dev, CS_DEVCLASS_CPU) != 0) {
...
#else
      {
        ...
        int is_ptm = (meta.etm_version >= 0x40 && meta.etm_version < 0x50);
        cs_for_each_device(pd)
        {
          if (cs_device_has_class(pd, CS_DEVCLASS_SOURCE|CS_DEVCLASS_CPU) &&
              cs_device_get_affinity(pd) < cs_device_get_affinity(dev)) {
            struct etm_metadata pm;
            cs_get_etm_metadata(pd, &pm);
            if ((pm.etm_version >= 0x40 && pm.etm_version < 0x50) == is_ptm) {
              ++countp;
            }
          }
        }
        ...
        if (name_buf) {
            snprintf(name_buf, name_buf_size, "%s_%u", (is_ptm ? "PTM" : "ETM"), countp);
        }
      }
#endif
      ...
      if (meta.etm_version < 0x40) {
          /* e.g. 0x35 (from id 0x25) is ETM3.5 */
          add_to_buf(&b, "type=ETM%d.%d\n\n", (meta.etm_version >> 4) & 0xF, meta.etm_version & 0xF);
      } else if (meta.etm_version < 0x50) {
          /* e.g. 0x41 (from id 0x31) is PFT1.1 */
          add_to_buf(&b, "type=PTM%d.%d\n\n", ((meta.etm_version >> 4) & 0xF)-3, meta.etm_version & 0xF);
      } else {
          /* e.g. 0x50 is ETM4.0 */
      ...
      add_to_buf(&b, "[regs]\n");
      add_to_buf(&b, "ITM_CTRL(0x%03X)=0x%08X\n", CS_ITM_CTRL >> 2, cs_device_read(dev, CS_ITM_CTRL));
      if (name_buf) {
      ...
      add_to_buf(&b, "[regs]\n");
      add_to_buf(&b, "STMTCSR(0x%03X)=0x%08X\n", CS_STM_TCSR >> 2, cs_device_read(dev, CS_STM_TCSR));
      if (name_buf) {
          snprintf(name_buf, name_buf_size, "STM_%u", trace_id);
      }
    } else if (d_type == DEV_FTM) {
      add_to_buf(&b, "[device]\n");
      add_to_buf(&b, "name=FTM_%u\n", trace_id);
      add_to_buf(&b, "class=trace_source\n");
      add_to_buf(&b, "type=FTM\n\n");
      add_to_buf(&b, "[regs]\n");
      add_to_buf(&b, "ITM_CTRL(0x%03X)=0x%08X\n", CS_ITM_CTRL >> 2, cs_device_read(dev, CS_ITM_CTRL));
      if (name_buf) {
          snprintf(name_buf, name_buf_size, "FTM_%u", trace_id);
      }
    } else {
